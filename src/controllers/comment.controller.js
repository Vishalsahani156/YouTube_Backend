  ...
  const allComments = await Comment.aggregate([
    ...
    },
    {
      $lookup: {
        ...
        pipeline: [
          {
            $project: { fullName: 1, username: 1, avatar: 1, _id: 1 },
          },
        ],
      },
    },
    { $unwind: "$owner" },
    {
      $project: {
        ...
        createdAt: 1,
        updatedAt: 1,
        isOwner: { $cond: { if: { $eq: [req.user?._id, "$owner._id"] }, then: true, else: false } },
        likesCount: { $size: "$likes" },
        disLikesCount: { $size: "$dislikes" },
        isLiked: { $cond: { if: { $in: [req.user?._id, "$likes"] }, then: true, else: false } },
        isDisLiked: { $cond: { if: { $in: [req.user?._id, "$dislikes"] }, then: true, else: false } },
        isLikedByVideoOwner: { $cond: { if: { $in: [video.owner, "$likes"] }, then: true, else: false } },
      },
    },
  ]);

  return res.status(200).json(new ApiResponse(200, allComments, "All comments sent"));
});
...
const addComment = asyncHandler(async (req, res) => {
  ...
  return res.status(200).json(new ApiResponse(200, commentData, "Comment added successfully"));
});
...
const updateComment = asyncHandler(async (req, res) => {
  ...
  if (!content || content.trim().length === 0) throw new ApiError(400, "Content cannot be empty");
